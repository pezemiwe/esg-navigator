...
const hooksInsertion = \  const [s1DataState, setS1DataState] = useState<any[]>([]);
...
if(!txt.includes('const [s1DataState')) {
    txt = txt.replace('const [s3Modal, setS3Modal] = useState(false);', 'const [s3Modal, setS3Modal] = useState(false);\n' + hooksInsertion);
}

const oldHeaderRegex = /<div className="bg-white border border-\[\#e0e0e0\] flex items-center justify-between px-6 py-6 mb-6">[\s\S]*?<\/div>\s*<\/div>/;

const replacementHeader = \
        <div className="bg-white border border-[#e0e0e0] flex flex-col px-6 py-6 mb-6">
            <div className="flex items-center justify-between mb-6">
                <div>
                <h2 className="text-[20px] font-normal text-[#161616]">PCAF Financed Modules (Scope 3)</h2>
                <p className="text-[14px] text-[#525252] mt-1">Upload the respective datasets to automatically cascade through Option 1-4 calculations.</p>
                </div>
                        ...
                        <input
                            ...
                            onChange={async (e) => {
                                ...
                                try {
                                    ...
                                    const json = XLSX.utils.sheet_to_json(workbook.Sheets[first]);
                                    sheet.setter(json);
                                } catch (err) {
                                    console.error(err);
                                } finally {
...
txt = txt.replace(/<div className="bg-white border border-\[\#e0e0e0\] flex items-center justify-between px-6 py-6 mb-6">[\s\S]*?<\/div>(\s*)<\/div>/, replacementHeader + '');
