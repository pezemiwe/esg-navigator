with open('src/features/sustainability/pages/EmissionsModule.tsx', 'r', encoding='utf-8') as f:
    txt = f.read()

target = '<div className=\"bg-white border border-[#e0e0e0] flex items-center justify-between px-6 py-6 mb-6\">'
s_idx = txt.find(target)
e_idx = txt.find('{/* STEP 4: DASHBOARD / METRICS */}', s_idx)
...
if s_idx != -1 and e_idx != -1:
    out = txt[:s_idx] + replacement + txt[e_idx:]
    
    hooks = '''  const [s1DataState, setS1DataState] = useState<any[]>([]);
  ...
  '''
    if "const [s1DataState," not in out:
        out = out.replace('const [s3Modal, setS3Modal] = useState(false);', 'const [s3Modal, setS3Modal] = useState(false);\n' + hooks)
        
    with open('src/features/sustainability/pages/EmissionsModule.tsx', 'w', encoding='utf-8') as f:
        f.write(out)
    print('done')
else:
