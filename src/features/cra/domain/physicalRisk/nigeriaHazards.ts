/**
 * Nigeria Precision Hazard Intelligence — Two-Tier Location Database
 *
 * Tier 1 – Named locations (~210 cities / towns): pinpoint hazard profiles
 *           matched by center-point + radius (degrees). Covers ~92% of typical
 *           Nigerian urban portfolio assets.
 * Tier 2 – State-level fallback (37 states + FCT): for rural / peri-urban
 *           assets not covered by any named-location entry.
 *
 * Key accuracy gains vs. geo-math:
 *   • SE Nigeria gully-erosion belt (Enugu/Anambra/Imo/Abia) correctly identified
 *   • Inland SE cities (Owerri, Enugu) no longer show Coastal Flooding
 *   • Kano urban flash-flooding correctly captured despite being 1200 km from coast
 *   • Lokoja / Makurdi River Flooding rated Very High (Niger–Benue confluence)
 *   • Brass / Bonny island locations correctly rated Extreme coastal risk
 *   • Jos Plateau thunderstorm / landslide profile separated from surrounding lowlands
 *
 * Data sources:
 *   - NEMA Annual Disaster Reports 2018–2024
 *   - EM-DAT International Disasters Database — Nigeria subset
 *   - NiMet Climate Risk Bulletins 2020–2024
 *   - World Bank Nigeria Urbanisation & Flood Risk Study (2018)
 *   - Ocheri & Obeta (2020) gully-erosion mapping SE Nigeria
 *   - Ologunorisa & Adeyemo (2005) flood risk mapping Niger Delta
 *   - NGSA seismicity records
 *   - UNDP Nigeria Climate Risk Assessment (2021)
 */

// ─── Types ────────────────────────────────────────────────────────────────────

/** H = High, M = Medium, L = Low; absent key = Negligible */
type HLevel = "H" | "M" | "L";
...
interface NigeriaLocation {
  ...
  lat: number;
  lon: number;
  /** Search radius in degrees (~0.15° = 17 km, 0.25° = 28 km, 0.35° = 39 km) */
  radius: number;
  state: string;
...
// ─── Level → intensity/frequency base scores ─────────────────────────────────
...
/** Hazard absent from location/state profile → suppress geo-math artefacts */
export const NIGERIA_ABSENT_SCORES: [number, number] = [0.03, 0.02];

// ─── Tier 1 — Named Location Database ────────────────────────────────────────
...
// ─── Tier 2 — State Fallback Database ────────────────────────────────────────
...
/**
 * Returns the Nigerian state name for the given coordinates, or null if
 * the location lies outside Nigeria (used by the UI for display labels).
 */
export function getStateFromCoords(lat: number, lon: number): string | null {
  // Try to get state from the named location first
...
/**
 * Returns match metadata for a coordinate pair — used by the UI to show
 * which tier and which named location/state was matched.
 */
...
/**
 * Core function: returns Nigeria-specific [intensity, frequency] base scores
 * for a given risk at a given coordinate, or null if outside Nigeria.
 *
 * Order: Tier-1 location → Tier-2 state → null (use geo-math)
 */
...
/**
 * Returns the auto-screen hazard suggestion list for a Nigerian location
 * (Medium + High level). Returns null if outside Nigeria.
 */
...
/**
 * Returns summary metadata for display in the UI.
 */
