...
Report Date: ${new Date().toLocaleDateString(useRegionStore.getState().profile.locale, { year: "numeric", month: "long", day: "numeric" })}
...
${entityProfile.name} presents this comprehensive sustainability disclosure aligned with the International Financial Reporting Standards (IFRS) S1 and S2 requirements. As a leading ${useRegionStore.getState().profile.demonym} commercial bank with ${entityProfile.branches} branches, ${entityProfile.employees.toLocaleString()} employees, and a total loan book of ${formatNaira(entityProfile.loanBook)}, the Bank recognizes its responsibility as a significant financial intermediary in driving ${useRegionStore.getState().profile.country}'s transition to a low-carbon economy.
...
8.1 Key Performance Indicators
  ...
  • GHG emissions intensity: ${entityProfile.loanBook > 0 ? (totalEmissions / (entityProfile.loanBook / 1e9)).toFixed(2) : "N/A"} tCO₂e per ${useRegionStore.getState().profile.currencySymbol}B loan book
