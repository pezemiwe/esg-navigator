...
const eIdx = txt.indexOf('{/* STEP 4:', sIdx);

if (sIdx === -1 || eIdx === -1) {
    console.log("Could not find targets!", sIdx, eIdx);
    process.exit(1);
}

// walk back to start of line for sIdx
const realStart = txt.lastIndexOf('\n', sIdx);
const realEnd = txt.lastIndexOf('\n', eIdx);
...
let newTxt = txt.substring(0, realStart) + replacement + txt.substring(realEnd);
...
console.log('Patched final block successfully');
