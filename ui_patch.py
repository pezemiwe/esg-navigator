...
replacement = '''            {step === 3 && (
              <div className="animate-in fade-in duration-200 w-full mb-20 px-8">
                <div className="mb-6">
                  ...
                  {[
                    ...
                    <div
                      ...
                      className={`p-6 flex flex-col justify-between ${
                        stat.highlight ? "bg-[#f4fadc]" : "bg-white"
                      }`}
                    >
                      <p
                        className={`text-[12px] uppercase font-semibold mb-6 tracking-wide ${
                          stat.highlight ? "text-[#86bc25]" : "text-[#525252]"
                        }`}
                      >
                        {stat.label}
                      </p>
                      <div>
                        <p
                          className={`text-[36px] font-light leading-none ${
                            stat.highlight ? "text-[#435e12]" : "text-[#161616]"
                          }`}
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                  ...
                  <div className="col-span-8 bg-white border border-[#e0e0e0] p-6 h-[420px]">
                    ...
                    <div className="h-[300px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                          ...
                          ).map(([name, value]) => ({ name: name.slice(0, 20)+'...', value })).sort((a,b) => b.value - a.value).slice(0, 5)}
                          layout="vertical"
                        >
                          <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#e0e0e0" />
                          <XAxis type="number" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "#525252" }} />
                          <YAxis type="category" dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "#525252" }} width={140} />
                          <Tooltip
                            cursor={{ fill: "#f4f4f4" }}
                  ...
                  <div className="col-span-4 bg-white border border-[#e0e0e0] p-6 h-[420px]">
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                            data={Object.entries(
                              portfolioResults.reduce((acc, curr) => {
                                acc[`DQS ${curr.dqs}`] = (acc.get(`DQS ${curr.dqs}`) or 0) + 1;
                                return acc;
                              }, {} as Record<string, number>)
                            ...
                            dataKey="value"
                          >
                            {Object.keys(portfolioResults.reduce((acc, curr) => { acc[`DQS ${curr.dqs}`] = 1; return acc; }, {})).map((entry, index) => (
                              <Cell key={`cell-${index}`} fill={['#86bc25', '#0f62fe', '#da1e28', '#f1c21b', '#8a3ffc', '#000000'][index % 6]} />
                            ))}
                          </Pie>
                  ...
                  <div className="col-span-1 bg-white border border-[#e0e0e0] p-0">
                     ...
                     <div className="overflow-x-auto">
                        <table className="w-full text-left text-[13px]">
                          <thead className="bg-[#f4f4f4] text-[#525252] font-medium border-b border-[#e0e0e0]">
                            <tr>
                  ...
                  <div className="col-span-1 bg-white border border-[#e0e0e0] p-6 flex flex-col justify-center">
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="flex justify-between items-center p-4 bg-[#f4f4f4] border border-[#e0e0e0]">
                        <span className="text-[14px] text-[#525252]">Total Exposure ($m)</span>
                        <span className="text-[18px] font-light text-[#161616]">{formatNumber(portfolioResults.reduce((a,c) => a + c.exposure, 0))}</span>
                      </div>
                      <div className="flex justify-between items-center p-4 bg-[#f4f4f4] border border-[#e0e0e0]">
                        <span className="text-[14px] text-[#525252]">Portfolio Carbon Footprint (tCO2e / $m)</span>
                      ...
                      <div className="flex justify-between items-center p-4 bg-[#f4f4f4] border border-[#e0e0e0]">
                        <span className="text-[14px] text-[#525252]">Weighted Average Carbon Intensity (tCO2e / $m Revenue)</span>
                        <span className="text-[18px] font-light text-[#161616]">
...
replacement = replacement.replace('or 0) + 1', '|| 0) + 1').replace('acc.get(', 'acc[')

pattern = re.compile(r'\{\s*step === 3 && \([\s\S]*?(?=<div className="fixed bottom-0 right-0)', re.MULTILINE)
