...
constants = constants.replace(/import \{[\s\S]*?\} from "@mui\/material";/g, '');
constants = constants.replace('useTheme,', '');
constants = constants.replace('import { useTheme } from "../../theme/hooks/useTheme";', '');
            ...
            <div key={pillar} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-sm">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center opacity-80" style={{ color: "#86bc25", backgroundColor: "rgba(134, 188, 37, 0.1)" }}>
                    <Icon size={20} />
                  </div>
                  <div>
          ...
          <div className="p-4 md:p-8 text-center max-w-4xl mx-auto">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-8 text-left">
              {[
                ...
                { label: "Materiality", value: \`\${selectedMaterialTopicIds.length} topics\`, ready: selectedMaterialTopicIds.length > 0 },
                { label: "Emissions", value: totalEmissions > 0 ? \`\${formatNumber(totalEmissions)} tCO2e\` : "None", ready: totalEmissions > 0 },
                { label: "Scenarios", value: \`\${scenarioResults.length} models\`, ready: scenarioResults.length > 0 },
                { label: "Templates", value: \`\${templates.length} deployed\`, ready: templates.length > 0 },
              ].map(item => (
                <div key={item.label} className="p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 rounded-xl flex items-center gap-3">
                  {item.ready ? <CheckCircle2 size={16} className={\`text-emerald-500\`} /> : <AlertCircle size={16} className="text-slate-400" />}
                  <div>
                    <div className="uppercase tracking-wider text-[10px] font-bold text-slate-500">{item.label}</div>
                    <div className={\`text-sm font-bold \${item.ready ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'}\`}>{item.value}</div>
                  </div>
                </div>
              ))}
            </div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {reportImages.map(img => (
                      <div key={img.id} className="relative w-40 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700">
                        <button onClick={() => removeImage(img.id)} className="absolute top-1 right-1 bg-red-500 text-white w-6 h-6 rounded flex items-center justify-center hover:bg-red-600 z-10 transition-colors">
                          <X size={12} />
                        </button>
                        <img src={img.dataUrl} alt={img.name} className="w-full h-24 object-cover border-b border-slate-200 dark:border-slate-700" />
...
console.log('AIReport.tsx rebuilt correctly!');
